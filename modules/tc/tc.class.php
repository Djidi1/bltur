...
class tcModel extends module_model {
   ...
   /* ORDER BY DAYOFYEAR(dob),dob';*/
	...
	public function saveData($type,$params) {
		switch ($type){
			case 'gidNew': $table= 'tc_gids'; break;
			case 'busNew': $table= 'tc_bus'; break;
			case 'ctNew': $table= 'tc_countrys'; break;
			case 'mpNew': $table= 'tc_mp'; break;
			case 'cityNew': $table= 'tc_citys'; break;
			case 'locNew': $table= 'tc_locations'; break;
		}
		$keys = array();
		$values = array();
	...
	public function UpdateData($type,$params) {
		switch ($type){
			...
			case 'mpNew': $table= 'tc_mp'; break;
			case 'cityNew': $table= 'tc_citys'; break;
			case 'locEdit': $table= 'tc_locations'; break;
		}
		$set = array();
		foreach ($params as $key => $value){
			if ($key != 'id')
	...
	public function turList($order, $f_name, $country, $target, $id_type, $start_date, $end_date) {
		...
		$sql = 'SELECT t.`id`, t.`name`,`date`,`date_to`,c.`name` city_name, l.`name` loc_name,
				...
				WHERE `date` >= \''.$start_date.'\' AND `date` <= \''.$end_date.'\' ' . $fsql . $order_by.'
		  		LIMIT 0,100';
	//	stop($sql);
		$this->query ( $sql, $f_name, $f_city, $id_loc );
		$items = array ();
		while ( ($row = $this->fetchRowA ()) !== false ) {
			$row ['date'] = $this->mydate_to_dmy( $row ['date'] );
			$row ['date_to'] = $this->mydate_to_dmy( $row ['date_to'] );
...
class tcProcess extends module_process {
	...
	public function check_bank_results($Order_ID){
		...
		return array($result,$total,$date);
	}
...
*/		
		if ($action == 'BankReq') {
			...
			list($result,$total,$date) = $this->check_bank_results($Order_ID);

			if(empty($result)) {
				list($result,$total,$date) = $this->check_bank_results($Order_ID*1000);
			}
		...
		if ($action == 'turDel') {
			...
			$res = $this->nModel->turDel ( $Params ['tur_id'] );
		...
		if ($action == 'userDelete') {
			...
			$res = $this->nModel->userDelete ( $Params ['user_id'] );
		...
		if ($action == 'userBan') {
			...
			$res = $this->nModel->userBan ( $Params ['user_id'] );
		...
		if ($action == 'travelBan') {
			...
			$res = $this->nModel->travelBan ( $Params ['travel_id'] );
...
*/
		...
		if ($action == 'locDel') {
			...
			$res = $this->nModel->locDel ( $Params ['loc_id'] );
		...
		if ($action == 'mpNew') {
			...
			if ($name!=''){
				...
				foreach ($locations as $loc) {$form .= '<option value="'.$loc['id'].'">'.$loc['name'].'</option>';}
				$form .= '</select></p>';
		...
		if ($action == 'cityNew') {
			...
			if ($name!=''){
				...
				foreach ($countris as $item) {$form .= '<option value="'.$item['id'].'">'.$item['name'].'</option>';}
				$form .= '</select></p>';
				$form .= '<input class="ui-button ui-state-default ui-corner-all" type="button"
